...
def process_train_MAM_data(spec, myFiles, config=None):
    ...
    with torch.no_grad():
        ...
        for idx in range(len(spec_stacked)):
            ...
            if length_routine == 'CONSTANT':
                ...
                if valid_index_range<0:
                    ...
                    with open('bad_files.txt','a') as myFile:
                        ...
                        myFile.write('\n')
                chosen_index = torch.randperm(valid_index_range).data.cpu().numpy()[:proportion] # draw `proportion` samples from the range (0, valid_index_range) and without replacement (TODO: note that this can have overlap)
                #print(chosen_index)
                # mask to zero
                #Now do channel masking
            ...
            # TODO: Mask label was not put on chosen_feats!!!
