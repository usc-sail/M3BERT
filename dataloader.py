...
from downstream.dataloader.collate_fn import collate_fn
...
class MelDataset(LibriDataset):
    ...
    def __init__(self, run_mockingjay, file_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        ...
        #sss
        X_lens = self.table['length'].tolist()
        #TODO: goes from shortest to longest now
        #X.reverse()
        #X_lens.reverse()
        ...
        for x, x_len in zip(X, X_lens):
            ...
            #TODO: Clean up this code so it's using the actual batchsize (HALF_BATCHSIZE_TIME > 400)    
            # Fill in batch_x until batch is full
            if len(batch_x) == bucket_size:
                # Half the batch size if seq too long
    ...
    def __getitem__(self, index):
        ...
        x_batch = [torch.FloatTensor(np.load(os.path.join(self.root, x_file))) for x_file in self.X[index]]
        #print(len(x_batch))
        #x_batch = [x_new[:,12:52] for x_new in x_batch]
        #print(x_batch[0].shape)
        #ssss
        x_pad_batch = pad_sequence(x_batch, batch_first=True)
        #xxxx
        #adapt_length = True
        adaptation = int((43*(index/len(self.X))+7)//1)
        #print(adaptation)
        if self.run_mockingjay: x_pad_batch = process_train_MAM_data(spec=(x_pad_batch,), myFiles=self.X[index], config=self.mock_config, adaptation=-1)
        return x_pad_batch


#BIG MTL
class MelMTLDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, label_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelMTLDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        assert(self.load == 'mtl')
        TAGS_GTZAN = [
            'blues',
            'classical',
            'country',
        ...
        X = self.table['file_path'].tolist()
        X_lens = self.table['length'].tolist()
        Y_gtzan = self.table['genre'].tolist()
        Y_mtg = self.table['moodtheme'].tolist()
        Y_rwc = self.table['RWC'].tolist()
        Y_eb = self.table['eb'].tolist()
        Y_deam = self.table['AV'].tolist()
        
        self.y_gtzan_encoder = MultiLabelBinarizer()
        self.y_gtzan_encoder.fit([TAGS_GTZAN])
        self.y_mtg_encoder = MultiLabelBinarizer()
        self.y_mtg_encoder.fit([TAGS_MTG])
        self.y_rwc_encoder = MultiLabelBinarizer()
        self.y_rwc_encoder.fit([TAGS_RWC])
        self.y_eb_encoder = MultiLabelBinarizer()
        self.y_eb_encoder.fit([TAGS_EB])
        

        self.class_num = [len(TAGS_GTZAN), len(TAGS_MTG), len(TAGS_RWC), len(TAGS_EB), 2]
        # Use bucketing to allow different batch size dillonknat run time
        self.X = []
        self.Y = []
        self.Y_weight = []
        batch_x, batch_y, batch_y_weight, batch_len = [], [], [], []
        for x, x_len, y_gtzan, y_mtg, y_rwc, y_eb, y_deam in zip(X, X_lens, Y_gtzan, Y_mtg, Y_rwc, Y_eb, Y_deam):
            ...
            #import ipdb
            #ipdb.set_trace()
            y_gtzan_item = self.y_gtzan_encoder.transform([str(y_gtzan).split('/')])
            y_mtg_item = self.y_mtg_encoder.transform([str(y_mtg).split('/')])
            y_rwc_item = self.y_rwc_encoder.transform([str(y_rwc).split('/')])
            y_eb_item = self.y_eb_encoder.transform([str(y_eb).split('/')])
            if pd.isna(y_deam):
                y_deam = '0.0/0.0'
            #else:
            #import ipdb
            #ipdb.set_trace()
            #print(y_deam)
            y_deam_item = np.array([[float(x) for x in y_deam.split('/')]])
            
            y_item = [y_gtzan_item, y_mtg_item, y_rwc_item, y_eb_item, y_deam_item]
            batch_y.append(np.concatenate(y_item, axis=1))
            
            y_weight_item = []
            for item in y_item:
                if item.sum() == 0:
                    y_weight_item.append(item)
                else:
                    y_weight_item.append(np.ones_like(item))
            #TODO: Adjust loss here
            #y_weight_item = [y_weight_item[0]*1.0,y_weight_item[1]*1.0,y_weight_item[2]*1.0,y_weight_item[3]*1.0/4.0,y_weight_item[4]*1.0/400.0]
            batch_y_weight.append(np.concatenate(y_weight_item, axis=1))
            ...
            if len(batch_x) == bucket_size:
                ...
                if (bucket_size >= 2) and (max(batch_len) > HALF_BATCHSIZE_TIME):
                    ...
                    self.Y.append(batch_y[:bucket_size//2])
                    self.Y.append(batch_y[bucket_size//2:])
                    self.Y_weight.append(batch_y_weight[:bucket_size//2])
                    self.Y_weight.append(batch_y_weight[bucket_size//2:])
                else:
                    self.X.append(batch_x)
                    self.Y.append(batch_y)
                    self.Y_weight.append(batch_y_weight)
                batch_x, batch_y, batch_y_weight, batch_len = [], [], [], []
    ...
    def __getitem__(self, index):
        # Load acoustic feature and pad
        x_batch = [torch.FloatTensor(np.load(os.path.join(self.root, x_file))) for x_file in self.X[index]]
        x_pad_batch = pad_sequence(x_batch, batch_first=True)      # torch.Size([batch_size, time, frequency])
        # spec_masked, pos_enc, mask_label, attn_mask, spec_stacked -> spec_stacked, pos_enc, attn_mask
        if self.run_mockingjay: 
            x_pad_batch = process_train_MAM_data(spec=(x_pad_batch,), myFiles=self.X[index], config=self.mock_config)
            _, pos_enc, _, attn_mask, spec_stacked = x_pad_batch
            x_pad_batch = (spec_stacked, pos_enc, attn_mask)
        y_batch = torch.cat([torch.Tensor(y_array) for y_array in self.Y[index]],dim=0)#torch.cat([torch.LongTensor(y_array) for y_array in self.Y[index]], dim=0)
        y_weight = torch.cat([torch.LongTensor(y_weight_array) for y_weight_array in self.Y_weight[index]], dim=0)
        y_batch_weight = {
            'labels': y_batch,
            'weight': y_weight
        }
        return x_pad_batch, y_batch_weight
...
class MelMTLDataset(LibriDataset):
    ...
    def __init__(self, run_mockingjay, file_path, label_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        ...
        self.class_num = [len(TAGS_EB), 2]
        # Use bucketing to allow different batch size dillonknat run time
        self.X = []
        self.Y = []
        self.Y_weight = []
        batch_x, batch_y, batch_y_weight, batch_len = [], [], [], []
        for x, x_len, y_eb, y_deam in zip(X, X_lens, Y_eb, Y_deam):
...
'''
################################################
# MEL MULTI-TASK LEARNING DATASET (GTZAN, MTG) #
################################################
'''
The Multi-Task Learning Dataset based on GTZAN and MTG
'''
...
class MelMTLDataset(LibriDataset):
    ...
    def __init__(self, run_mockingjay, file_path, label_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        ...
        self.class_num = [len(TAGS_GTZAN), len(TAGS_MTG)]
        # Use bucketing to allow different batch size dillonknat run time
        self.X = []
        self.Y = []
        self.Y_weight = []
        batch_x, batch_y, batch_y_weight, batch_len = [], [], [], []
        for x, x_len, y_gtzan, y_mtg in zip(X, X_lens, Y_gtzan, Y_mtg):
            ...
            y_gtzan_item = self.y_gtzan_encoder.transform([str(y_gtzan).split('/')])
            y_mtg_item = self.y_mtg_encoder.transform([str(y_mtg).split('/')])
            y_item = [y_gtzan_item, y_mtg_item]
            batch_y.append(np.concatenate(y_item, axis=1))
            
            y_weight_item = []
            #TODO: Adjust the loss here!
            for item in y_item:
                if item.sum() == 0:
                    y_weight_item.append(item)
                else:
    ...
    def __getitem__(self, index):
        ...
        if self.run_mockingjay: 
            ...
            _, pos_enc, _, attn_mask, spec_stacked = x_pad_batch
            x_pad_batch = (spec_stacked, pos_enc, attn_mask)
        y_batch = torch.cat([torch.LongTensor(y_array) for y_array in self.Y[index]], dim=0)
        y_weight = torch.cat([torch.LongTensor(y_weight_array) for y_weight_array in self.Y_weight[index]], dim=0)
        y_batch_weight = {
            'labels': y_batch,
            'weight': y_weight
...
#############################
# MEL GENRE DATASET (GTZAN) #
#############################
'''
The GTZAN Dataset based on LibriSpeech
'''
class MelGenreDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, label_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelGenreDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        assert(self.load == 'genre')
        TAGS = [
            'blues',
            'classical',
            'country',
        ...
        X = self.table['file_path'].tolist()
        X_lens = self.table['length'].tolist()
        Y = self.table['genre'].tolist()
        self.y_encoder = MultiLabelBinarizer()
        self.y_encoder.fit([TAGS])
        self.class_num = len(TAGS)
        ...
        for x, x_len, y in zip(X, X_lens, Y):
            batch_x.append(x)
            batch_len.append(x_len)
            batch_y.append(self.y_encoder.transform([y.split('/')]))
...
###########################
'''
The Autotagging Dataset based on LibriSpeech
'''
class MelDEAMDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelDEAMDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        #assert(self.load == 'autotagging'),
        
        self.run_mockingjay = run_mockingjay
        self.mock_config = mock_config
        X = self.table['file_path'].tolist()
        X_lens = self.table['length'].tolist()
        Y = self.table['AV'].tolist()
        #self.y_encoder = MultiLabelBinarizer()
        #self.y_encoder.fit([TAGS])
        self.class_num = 2
        ...
        for x, x_len, y in zip(X, X_lens, Y):
            batch_x.append(x)
            batch_len.append(x_len)
            batch_y.append([[float(x) for x in y.split('/')]])
            # Fill in batch_x until batch is full
            if len(batch_x) == bucket_size:
                # Half the batch size if seq too long
                if (bucket_size >= 2) and (max(batch_len) > HALF_BATCHSIZE_TIME):
    ...
    def __getitem__(self, index):
        ...
        x_pad_batch = pad_sequence(x_batch, batch_first=True)      # torch.Size([batch_size, time, frequency])
        # spec_masked, pos_enc, mask_label, attn_mask, spec_stacked -> spec_stacked, pos_enc, attn_mask
        if self.run_mockingjay: x_pad_batch = process_train_MAM_data(spec=(x_pad_batch,), myFiles=self.X[index], config=self.mock_config)
        _, pos_enc, _, attn_mask, spec_stacked = x_pad_batch
        x_pad_batch = (spec_stacked, pos_enc, attn_mask)
        #import ipdb
        #ipdb.set_trace()
        #print(self.Y[index])
        y_batch = torch.cat([torch.Tensor(y_array) for y_array in self.Y[index]], dim=0)
        return x_pad_batch, y_batch#, [x_file for x_file in self.X[index]]
...
The Autotagging Dataset based on LibriSpeech
'''
class MelRWCDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelRWCDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        #assert(self.load == 'autotagging'),
        TAGS = ['PF',
                'EP',
                'HC',
...
The Autotagging Dataset based on LibriSpeech
'''
class MelEBDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelEBDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        #assert(self.load == 'autotagging'),
        TAGS = ['Chacha',
                'Foxtrot',
                'Jive',
...
###########################
# MEL AUTOTAGGING DATASET #
###########################
'''
The Autotagging Dataset based on LibriSpeech
'''
class MelAutotaggingDataset(LibriDataset):
    
    def __init__(self, run_mockingjay, file_path, sets, bucket_size, max_timestep=0, max_label_len=0, drop=False, mock_config=None, load='spec'):
        super(MelAutotaggingDataset, self).__init__(file_path, sets, bucket_size, max_timestep, max_label_len, drop, load)
        #assert(self.load == 'autotagging'),
        TAGS = ['action',
                'adventure',
                'advertising',
...
def get_Dataloader(split, load, data_path, batch_size, max_timestep, max_label_len, 
                   ...
                   target_path=None, phone_path=None, label_path=None,
    ...
    if load in ['asr', 'text']:
        ...
        ds = MelEBDataset(run_mockingjay=run_mockingjay, file_path=data_path, sets=sets, max_timestep=max_timestep, load=load, 
                                   max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    elif load == 'rwc':
        ds = MelRWCDataset(run_mockingjay=run_mockingjay, file_path=data_path, sets=sets, max_timestep=max_timestep, load=load, 
                                   max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    elif load == 'deam':
        ds = MelDEAMDataset(run_mockingjay=run_mockingjay, file_path=data_path, sets=sets, max_timestep=max_timestep, load=load, 
                                   max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    elif load == 'autotagging':
        ds = MelAutotaggingDataset(run_mockingjay=run_mockingjay, file_path=data_path, sets=sets, max_timestep=max_timestep, load=load, 
                                   max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    elif load == 'genre':
        ds = MelGenreDataset(run_mockingjay=run_mockingjay, file_path=data_path, label_path=label_path, sets=sets, max_timestep=max_timestep, load=load, 
                             max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    elif load == 'mtl':
        ds = MelMTLDataset(run_mockingjay=run_mockingjay, file_path=data_path, label_path=label_path, sets=sets, max_timestep=max_timestep, load=load, 
                           max_label_len=max_label_len, bucket_size=bs, drop=drop_too_long, mock_config=mock_config)
    else:
