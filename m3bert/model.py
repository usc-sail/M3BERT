...
class M3BERTModel(M3BERTInitModel):
    ...
    def forward(self, spec_input, pos_enc, attention_mask=None, output_all_encoded_layers=True, head_mask=None):
        ...
        extended_attention_mask = extended_attention_mask.to(dtype=torch.float16)#next(self.parameters()).dtype) # fp16 compatibility
        ...
        if head_mask is not None:
            ...
            head_mask = head_mask.to(dtype=torch.float16) # switch to fload if need + fp16 compatibility
        else:
...
class M3BERTForMaskedAcousticModel(M3BERTInitModel):
    ...
    def forward(self, spec_input, pos_enc, mask_label=None, attention_mask=None, spec_label=None, head_mask=None):
        ...
        #TODO: This was a bug! I didn't do anything to fix it, needed to go to mam.py in utility
