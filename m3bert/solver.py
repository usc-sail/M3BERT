...
class Solver():
    ...
    def __init__(self, config, paras):
        ...
        #TODO: FIX THIS SO ALWAYS 324
        self.input_dim = 324#mel_dim if 'fmllr' not in config['dataloader']['data_path'] else fmllr_dim
    ...
    def set_model(self, inference=False, with_head=False, from_path=None, output_attention=False):
        ...
        if inference and not with_head:
            ...
            if self.apex:
                ...
                #TODO: sub out this for a real optimizer? Alternatively, use self.optimizer on LHS
                ...
                #print(self.warmup_proportion)
                #print(num_train_optimization_steps)
                #jjjjjjj
                #TODO: Was formerly num_train_optimization_steps in t_total
        ...
        if self.load or from_path is not None: # This will be set to True by default when Tester is running set_model()
    ...
    def load_model(self, inference=False, with_head=False, from_path=None):
        if from_path is not None:
            ...
            try:
                ...
                #print(all_states['Optimizer']['param_groups'][0]['schedule'])
                w = self.config['optimizer']['warmup_proportion']
                ts = self.config['solver']['total_steps']
                #print(w)
                #print(lr)
                gs = all_states['Global_step']
                my_lr = lr*(((gs+0.)/(ts))-1.)/(w-1.)
                #print(my_lr)
                #print(all_states['Global_step'])
                self.config['optimizer']['learning_rate'] = my_lr
                #print("got here")
                #sss
            except:
                print("Could not change the learning rate from another checkpoint")
    ...
    #TODO: FIX THE POSITIONAL ENCODING, IF NECESSARY!
...
class Trainer(Solver):
    ...
    def process_data(self, spec):
        ...
        with torch.no_grad():
            ...
            mask_label = torch.ByteTensor(mask_label).type(torch.bool).to(device=self.device)
            #yyy
            #print(mask_label)
    ...
    def exec(self):
        ...
        while self.global_step <= self.total_steps:
            ...
            for step, batch in enumerate(progress):
                ...
                #print(batch[3][0][0][0][0:])
                #print(np.shape(batch[0]))
                #print(np.shape(batch[1]))
                #print(batch[1])
                #print(np.shape(batch[2]))
                #print(np.shape(batch[3]))
                #print(np.shape(batch[4]))
                #print(np.shape(batch[4]))
                #print('')
                #xxxx
                ...
                #print(spec_masked[0][0][:])
                #jjj
                # Accumulate Loss
                if self.gradient_accumulation_steps > 1:
                    #TODO: don't divide by grad acc steps?
                    loss = loss / self.gradient_accumulation_steps
                if self.apex:
                    #TODO: WAS self.optimizer.backward(loss)
                    #self.optimizer.backward(loss.sum())
                    loss.sum().backward()
                else:
                    #TODO: WAS loss.backward()
                    #BEST: was loss.sum().backward()
                    #Could be loss.mean().backward()
                    loss.sum().backward()
                try:
                    # Update
                    ...
                    if self.global_step % self.log_step == 0:
                        # Log
                        #I changed this so apex wouldn't bug out
                        if self.apex:
                            self.logger.add_scalar('lr', lr_this_step, self.global_step)
                        else:
                            self.logger.add_scalar('lr', self.optimizer.get_lr()[0], self.global_step)
                        #print(loss.sum())
                        #TODO: Best was loss.sum().item()
                        self.logger.add_scalar('loss', loss.mean().item()*self.gradient_accumulation_steps, self.global_step)
                        self.logger.add_scalar('gradient norm', grad_norm, self.global_step)
                        #TODO: Can use mean as so: progress.set_description("Loss %.4f and loss sum %.4f" % (loss.mean().item(),loss.sum().item()))
                        #TODO: was loss.sum().item()
                        progress.set_description("Loss %.4f" % loss.mean().item())
                    ...
                    if self.global_step % self.save_step == 0:
                        ...
                        self.logger.add_image('mask_spec', mask_spec, self.global_step)
                        self.logger.add_image('pred_spec', pred_spec, self.global_step)
                        self.logger.add_image('true_spec', true_spec, self.global_step)
...
class Tester(Solver):
    ...
    def __init__(self, config, paras):
        ...
        # TODO: Xuan: here is confusing, Tester does not have parameters. Skip now.
        # print('Number of parameters: ' + str(sum(p.numel() for p in self.parameters() if p.requires_grad)))
            ...
            for x in tqdm(self.dataloader, desc="Plotting"):
                ...
                if with_head:
                    ...
                    for i in range(len(spec_masked)):
                        ...
                        #sample_index_feat = random.sample(range(324),128)
                        #spec_masked[i][:][sample_index_feat] = 0
